...
const CsvReader = () => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => setFiltersVisible(false);
  ...
  const sortedPapers = [...filteredPapers].sort((a, b) => {
    if (sortOrder === "asc") return a.Year.localeCompare(b.Year);
    if (sortOrder === "desc") return b.Year.localeCompare(a.Year);
    return 0;
  });
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4">
      {/* Filters Panel */}
      <div
        className={`md:w-1/4 bg-white fixed md:relative top-0 left-0 w-full z-10 transition-transform duration-300 ${
          filtersVisible ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0 h-full md:h-auto py-6 px-4`}
      >
      ...
      <div className="md:flex-1 w-full">
        <div
          className="cursor-pointer py-2 text-lg text-black md:hidden"
          onClick={toggleFiltersVisibility}
        >
          {filtersVisible ? (
        ...
        <div className="space-y-6 w-full">
          {sortedPapers.length > 0 ? (
            sortedPapers.map((row, index) => (
              ...
              >
                <div className="text-gray-500 space-x-4 mb-2 text-sm">
                  <span className="font-semibold text-black">{row.Paper}</span>
                  <span className="text-[#0064b1]">{row.Field}</span>
                  <span>{row.Year}</span>
                </div>
                <h2 className="text-black font-semibold text-xl mb-1 leading-tight">
                  <a
                    href={row.Link}
                    target="_blank"
