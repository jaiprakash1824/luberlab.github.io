...
const TypeFilter = ({ data, selectedType, setSelectedType, filterVisibility, toggleFilterSectionVisibility }) => {
  ...
  return (
    <div className="filter-option">
      ...
      {filterVisibility.type && (
        <>
          <div
            className={`sort-option ${selectedType.length === 0 ? 'active' : ''}`}
          ...
          {uniqueTypes.map((type) => (
            <div
              key={type}
              className={`sort-option ${selectedType.includes(type) ? 'active' : ''}`}
